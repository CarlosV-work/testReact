...
El objetivo de esta prueba técnica es que el candidato muestre sus habilidades con las herramientas que utilizará luego en su trabajo diario en Geotec. Está diseñado para verificar las habilidades de desarrollo front-end utilizando React y su capacidad para resolver problemas.
...
Para cada página debería leer los datos desde el archivo JSON [feed/videoteca.json](./data/videoteca.json), luego:

- El archivo JSON deberá cargarse en algun servidor local como el [JSON Server](https://cafeparadesarrolladores.com/json-server-y-node-js/)
- Despues de hacer correr el servidor local, mostrar los primeros 20 resultados. No es necesario paginar. 
...
También necesitará manejar los estados de carga/loading y error de obtener los datos desde el archivo JSON de su localhost:
...
- Como empresa, creemos que la comunicación es la clave del éxito. Entonces, si algo no está claro, o si tiene dudas sobre la tarea, consúltanos!
